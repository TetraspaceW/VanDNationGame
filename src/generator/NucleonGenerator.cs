...
class NucleonGenerator
{
    ...
    public TileModel[,] GenerateNucleon()
    {
        ...
        TerrainGenRule.Fill(parent, nucleonMap, [new TerrainRule(Terrain.TerrainType.GluonSea)]);
        ...
        .Zip(quarks, (colour, flavour) =>
            {
                TerrainGenRule.AddOneRandomly(parent, nucleonMap, [ new TerrainRule(Terrain.TerrainType.ValenceQuark, true, props: new Dictionary<PropKey, string>() {
                    {PropKey.QuarkColour, colour.ToString()},
                    {PropKey.QuarkFlavour, flavour.ToString()}
                }) ], [Terrain.TerrainType.ValenceQuark]);
                return true;
            }
        ).ToList();
